...
func EchoFrame() {
	...
	// ############################################################
	echo_tmpl, err := template.New("RenderData").Funcs(FuncMap).Parse(devechoTemplate)
	if err != nil {
		panic(err)
	}
...
var (
	...
	app_otel    string
	app_tls     string
	{{.AppName | replaceString}}devechocli= &cobra.Command{
		Use:   "run",
		Short: "Run Development server ",
		Long:  {{.BackTick}}Run development server{{.BackTick}},
...
func SetupRoutes(app *echo.Echo, test bool) {
	...
	{{range .Relations}}
	gapp.GET("/{{.LowerFieldName}}{{.LowerParentName}}/:{{.LowerFieldName}}_id/:{{.LowerParentName}}_id",controllers.Get{{.FieldName}}{{.ParentName}}s).Name = "get_{{.LowerFieldName}}{{.LowerParentName}}"
